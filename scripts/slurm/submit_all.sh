...
echo "[3/4] Submitting: fetch_arb_dfs (array 1-${TOTAL_CASES}, max 2 concurrent)"
ARB_DFS_JOB=$(sbatch --export="$EXPORT_VARS" \
    --dependency=afterok:${UPDATE_JOB} \
    --array=1-${TOTAL_CASES}%2 \
    --parsable \
...
# ---------------------------------------------------------------------------
echo "[4/4] Submitting: fetch_lifecycle (array 1-${TOTAL_CASES}, max 2 concurrent)"
